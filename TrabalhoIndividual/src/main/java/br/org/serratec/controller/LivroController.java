...
public class LivroController {
	...
	public ResponseEntity<Livro> buscarLivro(@PathVariable Long id) {
		return livroRepository.existsById(id)?ResponseEntity.ok(livroRepository.findById(id).get()):ResponseEntity.notFound().build();
	...
	public ResponseEntity<Livro> atualizarLivro(@PathVariable Long id, @Valid @RequestBody Livro livro) {
		livro.setId(id);
		return livroRepository.existsById(id)?ResponseEntity.ok(livroRepository.save(livro)):ResponseEntity.notFound().build();
